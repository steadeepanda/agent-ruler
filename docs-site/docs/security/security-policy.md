...
## Supported Versions
`v0.1.7` (Linux-first) is currently maintained.
...
- v0.1.7 enforcement requires launching via Agent Ruler runner/API flows.
...
Limitation note:
- v0.1.7 does not provide syscall-complete outbound mediation for arbitrary binaries without kernel-level hooks; command preflight primarily covers explicit URL-bearing command patterns.
