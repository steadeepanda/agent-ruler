//! Claude Code-specific preflight endpoint wrapper.
...
pub async fn api_claudecode_tool_preflight(
    State(state): State<WebState>,
    Json(payload): Json<OpenClawToolPreflightPayload>,
) -> impl IntoResponse {
    run_tool_preflight_for_runner(state, payload, RunnerKind::Claudecode.id()).await
}
