//! OpenClaw tool preflight endpoint helpers.
//!
//! Extracted from `src/ui.rs` to keep route wiring separate from OpenClaw-specific
//! policy mediation. When this file changes, check the `/api/openclaw/tool/preflight`
//! route declaration in `src/ui.rs`.
//!
//! Security boundary:
//! - This endpoint exists to mediate native OpenClaw tools before side effects happen.
//! - It appends receipts for every decision so timeline/debug views reflect blocked actions.
//! - It intentionally maps only known file/exec tool families; unknown tools are ignored.
...
/// Evaluate an OpenClaw core tool attempt before execution and persist a receipt.
///
/// The response model is intentionally compact so the OpenClaw `before_tool_call`
/// hook can map it directly to `block` / `blockReason` semantics.
pub async fn api_openclaw_tool_preflight(
    ...
    if decision.verdict == Verdict::RequireApproval {
        match approvals.has_active_approval_for(&action) {
            Ok(true) => {
                decision = Decision {
                    verdict: Verdict::Allow,
                    reason: ReasonCode::AllowedByPolicy,
    ...
    if let Err(err) = append_receipt(
        ...
        return error_response(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("append receipt for openclaw tool preflight: {err}"),
        );
    }
    ...
    match decision.verdict {
        ...
        Verdict::RequireApproval => {
            let pending = match approvals.create_pending(
                &action,
                &decision,
                "openclaw tool action requires approval",
            ) {
                Ok(value) => value,
                Err(err) => {
                    return error_response(
                        StatusCode::INTERNAL_SERVER_ERROR,
                        format!("create pending approval for openclaw tool preflight: {err}"),
                    );
                }
            };
            (
...
fn build_openclaw_tool_action(
    ...
    let mut action = ActionRequest {
        ...
        operation: format!("openclaw_tool_{tool_name}"),
        path: None,
        secondary_path: None,
        host: None,
        metadata,
        process: ProcessContext {
            pid: std::process::id(),
            ppid: None,
            command: format!("openclaw_tool {tool_name}"),
    ...
    match tool_name {
        "write" | "edit" | "append" | "apply_patch" => {
            ...
            action.path = Some(normalize_openclaw_tool_path(runtime, &path));
        }
        "delete" | "remove" | "rm" => {
            ...
            action.path = Some(normalize_openclaw_tool_path(runtime, first));
            action
                .metadata
        ...
        "move" | "rename" => {
            ...
            action.path = Some(normalize_openclaw_tool_path(runtime, &destination));
            action.secondary_path = Some(normalize_openclaw_tool_path(runtime, &source));
        }
        "read" | "cat" => {
            let path = extract_first_param_string(
                params,
        ...
        "exec" | "bash" | "run_command" => {
            ...
            if let Some(info) = copy_move_info {
                ...
                action.path = Some(normalize_openclaw_tool_path(runtime, &info.dst_path));
                action.secondary_path = Some(normalize_openclaw_tool_path(runtime, &info.src_path));
                action.metadata.insert(
                ...
                action.process.command = format!("openclaw_tool {tool_name} {command_text}");
                ...
                action.path = Some(normalize_openclaw_tool_path(runtime, &info.target_path));
                action.metadata.insert(
                    "underlying_exec".to_string(),
                    resolved.to_string_lossy().to_string(),
                );
                ...
                action.path = Some(normalize_openclaw_tool_path(
                    runtime,
            ...
            } else if let Some(redirection_target) = extract_shell_redirection_target(&command_text)
            {
                ...
                action.kind = ActionKind::FileWrite;
                action.path = Some(normalize_openclaw_tool_path(runtime, &redirection_target));
                action.metadata.insert(
                    "underlying_exec".to_string(),
                    resolved.to_string_lossy().to_string(),
                );
...
fn normalize_openclaw_tool_path(runtime: &RuntimeState, raw: &str) -> PathBuf {
    let expanded_home = expand_home_path(raw);
    let candidate = expanded_home.unwrap_or_else(|| PathBuf::from(raw));
    if candidate.is_absolute() {
        candidate
    } else {
        runtime.config.workspace.join(candidate)
    }
}
